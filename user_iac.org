# Time-stamp: <2023-02-25 10:03:52 angelv> 
...
Here at IAC we have several %newwin%[[Supercomputing -> http://research.iac.es/sieinvens/SINFIN/Main/supercomputing.php]] resources that allow you to obtain your computational results in much less time and/or work with much more complex problems. One of them is %newwin%[[Condor -> http://research.cs.wisc.edu/htcondor/]], or HTCondor since it is a High Throughput Computing (%newwin%[[http://en.wikipedia.org/wiki/High-throughput_computing|HTC]]) system. The underlying idea is quite simple (and powerful): let's use idle machines to perform computations while their owners are away. So, in a nutshell, HTCondor is an application that is installed in our PCs to make it possible to run a large number of yours and others' computations at a time in different machines when they are not being used, achieving a better utilization of our resources. A more detailed overview of HTCondor is available at the official %newwin%[[documentation -> http://research.cs.wisc.edu/htcondor/overview/]] and in the old pages about  %newwin%[[http://research.iac.es/sieinvens/SINFIN/Condor/index.php | HTCondor at IAC]].
...
HTCondor has also other powerful features. For instance, you can also use it to [[create specific or periodic checkpoints -> http://research.iac.es/sieinvens/siepedia/pmwiki.php?n=HOWTOs.CondorFAQs#run_ckpt]] of your @@C@@, @@C++@@ or @@fortran@@ programs. Then you can normally run your application (without using HTCondor) and if something happens (your program crashes, PC is unexpectedly halted, etc.), you will be able to restart the execution from any of the checkpoints, saving a lot of time. Please, read this introduction and the other sections to discover how HTCondor can help you.
...
On the other hand, software specifications are quite homogeneous and all machines are running the same OS: Fedora Linux. Almost all machines run Fedora21 (as of June 2017, there are still a few machines running older versions, and a handful with Fedora 25). Installed software should be also more or less the same in every machine (see the %newwin%[[software supported by the SIE -> http://research.iac.es/sieinvens/SINFIN/Main/software_sinfin.php]]), which makes it easy to run almost every application in any machine (although the available software could be different in some machines that belong to the Instrumentation area). 

If your application has special requirements about memory per slot, OS version, etc., you can rank and/or limit these parameters and also a quite large set of other ones. ''Please, visit [[FAQs page -> CondorFAQs]] for more information and examples.''
...
If you need further information, please check the other pages about HTCondor at the SIEpedia: [[Useful Commands -> CondorUsefulCommands]], [[Submit Files (description and examples) -> CondorSubmitFile]], [[Submit Files (HowTo) -> CondorHowTo]], [[FAQs -> CondorFAQs]], , etc. HTCondor at SIEpedia is continuously updated, but we also have more documentation about older versions of HTCondor at the [[http://research.iac.es/sieinvens/SINFIN/Condor/index.php | the HTCondor section at IAC]] (most of that information is still valid, but some may be obsolete, including broken links). For detailed and complete information, check the %newwin%[[official documentation about HTCondor -> http://research.cs.wisc.edu/htcondor/manual/v8.6/]]. 

If you need help or you are having any kind of issues related to HTCondor, '''the SIE gives direct support''' to IAC's users who want to use HTCondor: we will not code your whole application, but we help and advise you about how to get the most out of HTCondor: use its commands, create submit files, modify your application to run it with HTCondor (in case it is needed), fix common mistakes, etc. We also organize workshops about HTCondor for IAC's users (the last one was on February, 25th 2014 - %newwin%[[slides available -> https://docs.google.com/presentation/d/1PqCih4yL6D3FOFo0W336RjLCKrSICZbYuPX1F1CZqtQ/present#slide=id.p]]), and we can organize a new workshop on demand if you and your colleges need it: if the group is large enough -10 or 12 people-, just contact us!). Even if HTCondor is not suitable for your needs, we may help you to get your results in less time using other %newwin%[[Supercomputing resources at IAC -> http://research.iac.es/sieinvens/SINFIN/Main/supercomputing.php]], like %newwin%[[http://en.wikipedia.org/wiki/High-performance_computing|High Performance Computing (HPC)]] in %newwin%[[http://www.iac.es/servicios.php?op1=30&lang=en | LaPalma Supercomputer]], etc.

>>frame bgcolor=#FAF2CC padding=6px<<
'''Contact data''': Carlos Luque: [[mailto:carlos.luque@iac.es | carlos.luque@iac.es]] | Phone ext.: 5547 | Office 3505 
>><<
...
**** Useful Commands
(:title HTCondor(2): Useful Commands:)

>>frame<<
(:table border=0 cellpadding=5 cellspacing=0 width=100% :)
(:cell align=center valign=middle :) [[Introduction -> Condor]]
(:cell align=center valign=middle :) '''Useful Commands'''
...
HTCondor has several dozens of commands, but in this section we will present just the most common ones (if you want to check the complete list, try the %newwin%[[Command Reference page -> http://research.cs.wisc.edu/htcondor/manual/v8.6/11_Command_Reference.html]]). Also remember that you can get further information running @@man condor_<cmd>@@ in your shell or visiting the %newwin%[[official Users' Manual -> http://research.cs.wisc.edu/htcondor/manual/v8.6/2_Users_Manual.html]]. The main command will be shown together with some useful options that may help work with HTCondor:
...
!! Submitting jobs
*@@'''condor_submit''' <submit_file>@@: Submit jobs to the HTCondor queue according to the information specified in @@submit_file@@. ''Visit the [[submit file page -> CondorSubmitFile]] to see some examples of these files. There are also some [[FAQs -> CondorFAQs]] related to the submit file.''
->''Useful options:''
...
(:cell align=center valign=middle :) '''Submit files (desc. & examples)'''
(:cell align=center valign=middle :) [[Submit files (HowTo) -> CondorHowTo]]
(:cell align=center valign=middle :) [[FAQs -> CondorFAQs]]
(:cell align=center valign=middle :) [[HTCondor and IDL -> CondorAndIDLVirtualMachine]]
(:tableend:)
