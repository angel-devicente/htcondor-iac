# Time-stamp: <2023-06-09 10:19:03 angelv> 
...
#  but this opens a new taw everytime I move in the table of contents, so we disable it
...
#+HTML: <details> <summary><b>Details</b></summary>
#+begin_quote
This manual is only a brief introduction to all HTCondor has to offer. For
detailed and complete information, check the official [[https://htcondor.readthedocs.io/en/v10_0/][HTCondor v10.0 Manual]].
...
* TODO TO DOs [3/13]                                               :noexport:
  + [X] Pre-introduction: date of FEDER funds?
  ...
  + [-] clean out old pages [2/3]
    + [ ] http://research.iac.es/sieinvens//SINFIN/Main/supercomputing.php is OK
      now, but the link to HTCondor@IAC should point to the HPC page, once the
      whole information has been processed
    + [X] quitar sección 4.1 de http://pasa
    + [X] quitar referencia a nectarino en older pages
https://angel-devicente.github.io/htcondor-iac/     + [X] quitar referencia a carlota en older pages
    + [X] process original HTcondor pages [10/10]
      + [X] code of conduct
        not necessary for the time being
      + [X] condor primer
      + [X] condor manual
      + [X] talk on condor
      + [X] hands-on session with condor
      + [X] workbook
      + [X] code examples
      + [X] code exercises
      + [X] questionnaire results
      + [X] condor pool statistics
      
    + [ ] process SIEpedia content
    + [ ] new content to add [0/4]
      + [ ] example with requirements / rank
        #+begin_example
        ############################
        #
        # complex.submit
        ...
        #+end_example
      + [ ] python bindings
      + [ ] take old PDF for Condor talk and modernize
        old_content/presentacion_condor.pdf
      + [ ] organize a Condor workshop after modernizing 
        old_content/course
