...
advantage of the htcondor module readily available though the module command in the updated "burros" as in:
...
to use the module load command on each remote machine (so you are sure the environment is homogeneus) a useful
bash script can be executed before (called pre_cmd.sh and has to be executable):
