# Time-stamp: <2023-10-22 07:16:25 angelv> 
...
[[http://pasa/ganglia/?r=week&cs=&ce=&m=load_one&tab=v&vn=pool_usage&hide-hf=false][here]]. Figure [[weekly_usage]] is an example showing the global usage where /Owner/
represents /slots/ that are being used outside of HTCondor. The remaining
/slots/ are available to HTCondor, but if there are no eligible jobs to run,
those slots will be in /CPUsNotInUse/ state. Those /slots/ that are actually
being used by HTCondor are those in the /CPUsInUse/ state.
...
=condor_status= (see Section [[*Checking pool status][Checking pool status]]). 
...
example: memory per slot, OS version, etc. (see Section [[*Submit files][Submit files]]).
...
and outputs, etc. (see Section [[*Submitting jobs][Submitting jobs]]).
...
In section [[*Basic HTCondor workflow][Basic HTCondor workflow]] we saw a very simple /submit
...
section [[*Basic HTCondor workflow][Basic HTCondor workflow]]. The commands used were already introduced in
section [[*Submit commands][Submit commands]]. For convenience we repeat below the /submit file/
(=simple.submit=):
...
The example in the previous section [[*HTCondor /Hello World!/][HTCondor Hello World!]] used the variable
...
tasks introduced in section [[*Basic HTCondor workflow][Basic HTCondor workflow]]: 
[[*Checking pool status][Checking pool status]], [[*Submitting jobs][Submitting jobs]], [[*Checking and managing submitted jobs][Checking and managing submitted jobs]] and [[*Getting information from the logs][Getting information from the logs]]. For each command we also give a
...
+ If you want some general information about the HTCondor Pool, where jobs have
  been executed on, etc., as mentioned in section [[*How *powerful* is HTCondor?][How powerful is HTCondor?]],
...
+ the description of a job is done via a =Submit= object. This is, in essence,
  ...
  commands can be used (see section [[*Submit files][Submit files]]).
...
+ this example illustrates how to use Python to submit jobs, where each job is
  ...
  + at the same time we also show how we can use software not installed in
    ...
    [[../software/modules.html][Environment Modules]] documentation). This is done by using the =job.sh=
    wrapper script, and it is further explained in 
    section [[*HTCondor and Environment Modules][HTCondor and Environment Modules]].
...
+ The job that we are submitting for this recipe uses the same *C* source file
  as that described in section [[*Basic HTCondor workflow][Basic HTCondor workflow]], so you just have to
...
makes it difficult to access (see section [[*HTCondor and Kerberos][HTCondor and Kerberos]]) your shell
...
#+begin_mdframed
#+INCLUDE: "../examples/python_bindings/job.sh" src bash
#+end_mdframed

and set the HTCondor =executable= command to =job.sh=, and its =arguments= to
=job.py [args]=. For a worked-out example of this, see section [[*HTCondor from Python][HTCondor from Python]].  
