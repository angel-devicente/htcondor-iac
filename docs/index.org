# Time-stamp: <2023-09-26 12:09:07 angelv> 
...
detailed overview of HTCondor is available at the [[https://htcondor.readthedocs.io/en/v10_0/overview/index.html][official documentation]].
...
detailed description you can check the official documentation ([[https://htcondor.readthedocs.io/en/v10_0/users-manual/submitting-a-job.html][Submitting a Job]],
[[https://htcondor.readthedocs.io/en/v10_0/man-pages/condor_submit.html][=condor_submit=]]).
...
[[https://htcondor.readthedocs.io/en/v10_0/man-pages/condor_submit.html#submit-description-file-commands][submit description file commands]]), but in most situations you will only need a
...
+ *Highly recommended commands*
  ...
  + *=universe=* your job(s) *execution environment*. By default, the simplest
    ...
    =universe= might be necessarey. See [[https://htcondor.readthedocs.io/en/v10_0/users-manual/choosing-an-htcondor-universe.html][Choosing an HTCondor Universe]].
...
documentation you can check the [[https://htcondor.readthedocs.io/en/v10_0/users-manual/submitting-a-job.html#submitting-many-similar-jobs-with-one-queue-command][full syntax and further examples]]. 
...
HTCondor provides around 100 commands (see the [[https://htcondor.readthedocs.io/en/v10_0/man-pages/index.html][Command Reference Manual)]], but
...
+ *=condor_submit_dag <dag_file>=* Submit a DAG file, used to describe jobs
  with dependencies. See the section on [[https://htcondor.readthedocs.io/en/v10_0/users-manual/dagman-workflows.html][DAGMan Workflows]] for more information. 
...
If you are comfortable with Python, or even want to send python script in HTCondor the best way is surely to take
advantage of the htcondor module readily available though the module command in the Ubuntu 22.04 updated "burros" as in:

#+begin_example
% module load python/3.10 
#+end_example

The following recipe uses this module to send the HTCondor jobs (no need to even save a submit file!) and it send a 
python script that uses an input file and two parameters. This input file (parameters.ini) is then searched for a 
text key and then changes the numerical value following (can be useful in many cases!).
The resulting file with the same name is transferred to the original machine, in a separate directory for each job. 

#+begin_mdframed
#+begin_note
This recipe might be too complicated as it does many things some you might not want, adapt to your needs. The juicy
part is the "itemdata" iterator that consistson a list of dictionaries, the values are used to give each individual
job different values by substituting in the keys in the format $(key_value) of the submit_description() object. Two 
variables, $(Cluster) and $(Process) are given by HTCondor and can be used to differentiate between output directories
as in this example.
#+end_note
#+end_mdframed

The simple python script launched for each job reads (called =simple_update.py=):

/The files for this recipe are in directory [[https://github.com/angel-devicente/htcondor-iac/tree/main/recipes/condor_python][=condor_python=]]./
...
to activate the =module load= command on each remote machine (so you are sure each job environment is homogeneus) 
a useful bash script can be executed before (called =pre_cmd.sh= and has to be executable):
...
The =parameters.ini= input file is plain text with entries in the form:

#+begin_example
initial_gas_density =  3.200E+06 ! initial gas density [part/cm-3]
...
cases you will be able to by using the [[https://htcondor.readthedocs.io/en/v10_0/users-manual/file-transfer.html][HTCondor's File Transfer Mechanism]]. 
#+end_warning
#+end_mdframed
...
detailed and complete information, check the official [[https://htcondor.readthedocs.io/en/v10_0/][HTCondor v10.0 Manual]].
