# Time-stamp: <2023-10-20 10:57:27 angelv> 

#+TITLE:   HTCondor@IAC User's Manual
#+AUTHOR:  Ángel de Vicente
#+EMAIL:   angel.de.vicente@iac.es

# LOAD minor mode: unpackaged/org-export-html-with-useful-ids-mode
#  otherwise the anchors are random orgxxxxx IDs
# Load only to generate the HTML and disable afterwards, otherwise I will get
#  trouble generating PDFs from Orgs.

# For ReadtheDoc clone
#+SETUPFILE: https://fniessen.github.io/org-html-themes/org/theme-readtheorg.setup

# From https://emacs.stackexchange.com/questions/14320/org-mode-links-open-in-new-tab/46383#46383
#  but this opens a new tag everytime I move in the table of contents, so we disable it
...
# Notes and frames
#  Note (as tip, warning, etc.) work for the HTML backend, but it is ignored by
#   PDF
#  mdframed works for the PDF backend, but it is ignored by HTML
#  putting both we get a frame in both backends

#+begin_mdframed
#+begin_note
This documentation, together with example submit files and other material can be
found in the [[https://github.com/angel-devicente/htcondor-iac][HTCondor@IAC]] repository.
#+end_note
#+end_mdframed
...
detailed overview of HTCondor is available at the [[https://htcondor.readthedocs.io/en/latest/overview/index.html][official documentation]].
...
You can see real-time *HTCondor@IAC statistics* (global and per user) [[http://pasa/ganglia/?r=week&cs=&ce=&m=load_one&tab=v&vn=pool_usage&hide-hf=false][here]]. Figure
[[weekly_usage]] is an example showing the global usage where /Owner/ represents
...
#+name: weekly_usage
#+caption: Weekly global usage
[[file:images/introduction/weekly_usage.png]]
...
machines make up the HTCondor@IAC pool at any given time in the [[http://pasa/ganglia/?r=week&cs=&ce=&c=IAC+HTCondor&h=&tab=m&vn=&hide-hf=false&m=cpu_num&sh=1&z=small&hc=4&host_regex=&max_graphs=0&s=by+name][HTCondor Ganglia
webpage]]. 
...
every machine (see the [[http://research.iac.es/sieinvens/SINFIN/Main/software_sinfin.php][software supported by the SIE]]), which makes it easy to
run almost every application in any machine.

Detailed information about each machine can be obtained with the command
=condor_status= (see Section [[*Checking pool status]]). 
...
example: memory per slot, OS version, etc. (see Section [[*Submit files]]).
...
and outputs, etc. (see Section [[*Submitting jobs]]).
...
+ *Submitting a job*
  ...
  + *your application*
    ...
    #+begin_mdframed
    #+INCLUDE: "../examples/hello_world/simple.c" src C
    #+end_mdframed
  ...
  + *a /submit file/*
    ...
    #+begin_mdframed
    #+INCLUDE: "../examples/hello_world/simple.submit" example
    #+end_mdframed
...
In section [[*Basic HTCondor workflow]] we saw a very simple /submit
...
detailed description you can check the official documentation ([[https://htcondor.readthedocs.io/en/latest/users-manual/submitting-a-job.html][Submitting a Job]],
[[https://htcondor.readthedocs.io/en/latest/man-pages/condor_submit.html][=condor_submit=]]).
...
[[https://htcondor.readthedocs.io/en/latest/man-pages/condor_submit.html#submit-description-file-commands][submit description file commands]]), but in most situations you will only need a
...
+ *Highly recommended commands*
  ...
  + *=universe=* your job(s) *execution environment*. By default, the simplest
    ...
    =universe= might be necessarey. See [[https://htcondor.readthedocs.io/en/latest/users-manual/choosing-an-htcondor-universe.html][Choosing an HTCondor Universe]].
...
the [[https://github.com/angel-devicente/htcondor-iac][HTCondor@IAC]] repository. 
#+end_note
#+end_mdframed
...
section [[*Basic HTCondor workflow]]. The commands used were already introduced in
section [[*Submit commands]]. For convenience we repeat below the /submit file/
(=simple.submit=):
...
The example in the previous section [[*HTCondor /Hello World!/]] used the variable
...
documentation you can check the [[https://htcondor.readthedocs.io/en/latest/users-manual/submitting-a-job.html#submitting-many-similar-jobs-with-one-queue-command][full syntax and further examples]]. 
...
#+begin_mdframed
#+INCLUDE: "../examples/queue_similar/simple.submit" example
#+end_mdframed
...
HTCondor provides around 100 commands (see the [[https://htcondor.readthedocs.io/en/latest/man-pages/index.html][Command Reference Manual)]], but
...
tasks introduced in section [[*Basic HTCondor workflow]]: Checking pool status
([[*Checking pool status]]), Submitting jobs ([[*Submitting jobs]]), Checking and
managing submitted jobs ([[*Checking and managing submitted jobs]]) and Getting info
from logs ([[*Getting information from the logs]]). For each command we also give a
...
+ There is also a useful online tool to analyze your log files and get more
  ...
  [[condor_log_analyzer]] shows an example of the output generated.

  #+name: condor_log_analyzer
  #+caption: Condor Log Analyzer sample output
  file:images/commands/condor_log_analyzer.png
...
+ *=condor_submit_dag <dag_file>=* Submit a DAG file, used to describe jobs
  with dependencies. See the section on [[https://htcondor.readthedocs.io/en/latest/users-manual/dagman-workflows.html][DAGMan Workflows]] for more information. 
+ *=condor_version=* Print the version of HTCondor.
+ If you want some general information about the HTCondor Pool, where jobs have
  been executed on, etc., as mentioned in section [[*How *powerful* is HTCondor?]],
  you can check real-time *HTCondor@IAC statistics* (global and per user) [[http://pasa/ganglia/?r=week&cs=&ce=&m=load_one&tab=v&vn=pool_usage&hide-hf=false][here]].  


** HTCondor from within Python

HTCondor comes with [[https://htcondor.readthedocs.io/en/latest/apis/python-bindings/tutorials/index.html][Python Bindings]], which provide a powerful mechanism to
...
[[https://htcondor.readthedocs.io/en/latest/apis/python-bindings/tutorials/Submitting-and-Managing-Jobs.html][here]], but as a quick example, the following script illustrates how you can use
...
#+begin_mdframed
#+INCLUDE: "../examples/python_bindings/submit.py" src python
#+end_mdframed
...
+ the description of a job is done via a =Submit= object. This is, in essence,
  ...
  commands can be used (see section [[*Submit files]]).
...
+ this example illustrates how to use Python to submit jobs, where each job is
  itself a Python code, so the executable file is in this case the =python3=
  interpreter itself, but it doesn't have to be so. You could specify any other
  type of executable file (for example a compiled code or a shell script as in
  the examples in section [[*Examples]].
  + at the same time we also show how we can use software not installed in
    ...
    interpreter which we would normally use in an interactive session my
    executing the command =module load python/3.10= (see the
    [[../software/modules.html][Environment Modules]] documentation). This is further explained in section
...
=recipes= directory of the [[https://github.com/angel-devicente/htcondor-iac][HTCondor@IAC]] repository. Bear in mind that these
...
foolproof, so if you find a better solution of any problems with any of them,
please let us know so we can try to fix them or find some other way around the
problem.
#+end_note
#+end_mdframed
...
cases you will be able to by using the [[https://htcondor.readthedocs.io/en/latest/users-manual/file-transfer.html][HTCondor's File Transfer Mechanism]]. 
#+end_warning
#+end_mdframed

/The files for this recipe are in directory [[https://github.com/angel-devicente/htcondor-iac/tree/main/recipes/condor_kerberos][=condor_kerberos=]]./
...
+ Create a Kerberos /keytab/ file, compatible with Active Directory
  ...
  The following information was taken from [[https://www.itadmintools.com/2011/07/creating-kerberos-keytab-files.html][here]]:
...
+ The job that we are submitting for this recipe uses the same *C* source file
  as that described in section [[*Basic HTCondor workflow]], so you just have to
...
makes it difficult to access (see section [[*HTCondor and Kerberos]]) your shell
init files (=.bashrc=, =.bash_profile=, etc.). Thus, you will likely run into
troubles with HTCondor if you are used to running [[../software/modules.html][Environment Modules]] (for
example, by running =module load python/3.10=) to prepare the environment for
your executions.

In these cases the best option (let us know if you find a better one) is
probably to manually specify the required changes to your environment variables
in order to mimic the behaviour of the =module load ...= command. While this
might sound as a daunting task, this is actually quite easy, as we only need to
see the relevant changes introduced in the environment variables when a
particular module is loaded, and once found we will use the same information for
any future HTCondor jobs requiring this module.

As an example, if we want to use the Python interpreter (and packages) provided
by the module =python/3.10= available in all IAC burros with Ubuntu22, we can
find the environment with and without the module loaded:
...
And if we then look at the differences (ignoring variables
=__MODULES_LMALTNAME=, =__MODULES_LMREFRESH=, =__MODULES_LMCONFLICT=,
=LOADEDMODULES=, and =_LMFILES_=, that are internal variables of Environment
Modules) we can see that the only variables modified are =PYTHONPATH= and
=PATH=): 

#+begin_src bash
$ diff env_base.txt env_python.txt 

  > PYTHONPATH=/opt/python/python3.10/lib/python3.10/site-packages
  < PATH=/usr/local/go/bin:[...]
  ---
  > PATH=/opt/python/python3.10/bin:/usr/local/go/bin:[...]
#+end_src

These environment changes can be added manually to the job description using the
=environment= command (see section [[*Submit commands]]). For a worked-out example
of this, see section [[*HTCondor from within Python]].
...
detailed and complete information, check the official [[https://htcondor.readthedocs.io/en/latest/][HTCondor Manual]].

If you need help or you are having any kind of issues related to HTCondor,
the [[http://research.iac.es/sieinvens/SINFIN/Main/index.php][SIE]] gives direct support to IAC's users who want to use HTCondor: we
...
commands at the *condor_submit page*. Just to
complement *some of the previous examples*, we will
...
+ For instance, if your username is =jsmith= and you want to specify a limit of
...
+ If you are executing jobs with *IDL without the IDL Virtual Machine*, then
...
http://research.cs.wisc.edu/htcondor/manual/v8.6/2_10DAGMan_Applications.html#SECTION003102000000000000000
Official documentation mentioned above. You can also try the easy example
located at the end of this page
http://research.iac.es/sieinvens/SINFIN/Condor/curso/course/node7.php used in a
course about HTCondor imparted by SIE some years ago (solution here
http://research.iac.es/sieinvens/SINFIN/Condor/curso/course/node9.php#SECTION00095000000000000000).
...
*** What is Condor? How can Condor help me? Who can use it? Who could help me if I have any problems? 
...
*introduction page* for more general information about Condor.
...
*** Sometimes Condor runs jobs on my computer when I am working on it, can I avoid that? 
...
*http://carlota:81/condor_stats/*. There you can get some stats about which
...
*** I am using Condor, should I add an acknowledgement text in my publications? 
...
*** I have developed a program, do I need to make any modification to run it with Condor? 
...
+ Your program should accept arguments, since changing arguments is the way used
  ...
  different data, paths to files or whatever your application use as input.
+ Paths to your input/output files may change when executing with Condor, so you
  should be able to change them in your application if needed.


*** How do I run my application with Condor? (submitting jobs to queue) 
...
Visit the *Useful commands page* to get more information about commands in
Condor.


*Caution!*: Before submitting your real jobs, always sdo ome simple tests in
...
*** How do I check the Condor queue and my submitted jobs? 
...
+ =*I=* ''idle'' job, waiting for a slot to be executed on (it can take a while
  before your jobs are executed, but if they are always in this state, check
  *this FAQ -> #idle*)
+ =*<=* your job is about to be executed, executable and input files are being
  transferred to the remote machine
+ =*R=* ''running'', your job is being executed at this moment
+ =*S=* ''suspended'', the machine that was running this job is being used,
  jobs are suspended while waiting for the machine gets idle again
+ =*>=* execution is finished, output files are being transferred
+ =*H=* ''on hold'', there are problems with your job that have to be solved
  (check *this FAQ -> #hel*)
+ =*<q*= or =*>q=* if you see those symbols, your transfers are waiting for the
...
*** I cannot submit jobs, what is wrong? 
...
*** My question is not in this list or I need further information, where can I find it? 

There are more *FAQs and How-to recipes ->
https://htcondor-wiki.cs.wisc.edu/index.cgi/wiki?p=HowToAdminRecipes* available
at Condor site and the *official Users' Manual ->
http://research.cs.wisc.edu/htcondor/manual/v8.6/2_Users_Manual.html* is useful,
too. Also you can visit other sections of Condor at the SIEpedia, like *useful
commands -> CondorUsefulCommands* or *Submit files -> CondorSubmitFile*. If you
need further information, please, contact us (you can find *here ->
Condor#contact* our contact data).



* HTCondor and IDL                                                 :noexport:
...
* TODO TO DOs [1/8]                                                :noexport:
  + [ ] Pre-introduction: date of FEDER funds?
  + [ ] Write info about policies burros vs desktops
  + [ ] Note: if you want to submit jobs from your desktop, it has to belong to
    the pool.
  + [ ] point out that only background processes!
  + [ ] HTCondor + IDL example
  + [ ] HTCondor + Apptainer example
  + [ ] Add more examples (see :noexport: sections)
  + [ ] Go through all :noexport: sections, clean and add if deemed useful 
  + [X] Point to the repository
